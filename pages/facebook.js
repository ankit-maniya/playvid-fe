...
const Facebook = () => {
  ...
  const handleBtnDownload = async (search) => {
    ...
    try {
      setLoading(true);
      const iRes = await VideoService.get_facebook({ url: search });
  ...
  const hasVideos = videoData && (videoData.formats || []).length > 0;
                ...
                <div className="flex-col lg:flex lg:flex-row">
                  ...
                  <div className="w-full mt-4 lg:mt-0 lg:w-2/3 lg:ml-2">
                    ...
                    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-2 gap-2">
                      {videoData.formats.map((video, idx) => (
                        <Link
                          key={idx}
                          href={video.url + "&dl=1"}
                          legacyBehavior
